...
class Train
  ...
  @@all_trains = [] # rubocop:disable Style/ClassVars
  ...
  def unhook_track(track)
    ...
    raise 'you can\'t unhook wagon, this train doesn\'t have any' if tracks.size.zero?
  ...
  def moove_to_the_next_statioin
    ...
    raise 'Train is alredy on the last station' if current_station == route.end_station
  ...
  def moove_to_the_previous_station
    raise "this train doesn't have a route yet" if route.nil?
    raise 'this train is alredy on the first station' if current_station == route.end_station
